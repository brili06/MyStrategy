...
export default function ExportOptions({
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.setFontSize(20);
      doc.text(`Strategic Management Analysis`, 105, yPosition, {
        align: "center"
      });
      yPosition += 10;
      doc.setFontSize(16);
      doc.text(`${type === "ife" ? "Internal" : type === "efe" ? "External" : "Internal-External"} Factor Evaluation Matrix`, 105, yPosition, {
        align: "center"
      });
      yPosition += 8;
      doc.setFontSize(10);
      ...
      if (swotItems.length > 0 && type === "ie") {
        ...
        // Create SWOT table
        const swotData = [["Strengths", "Weaknesses"], [swotItems.filter(item => item.category === "strength").map(item => item.description).join("\n\n"), swotItems.filter(item => item.category === "weakness").map(item => item.description).join("\n\n")], ["Opportunities", "Threats"], [swotItems.filter(item => item.category === "opportunity").map(item => item.description).join("\n\n"), swotItems.filter(item => item.category === "threat").map(item => item.description).join("\n\n")]];
      ...
      if (type === "ife" || type === "efe") {
        ...
        if (type === "ife") {
          // Strengths section
          const strengthRows = factors.filter(factor => factor.category === "strength").map((factor, index) => [index + 1, factor.description, factor.weight.toFixed(2), factor.rating, (factor.weight * factor.rating).toFixed(2)]);
          (doc as any).autoTable({
            head: [["", "Strengths", "", "", ""]],
            body: strengthRows,
            startY: yPosition,
          ...
          // Weaknesses section
          const weaknessRows = factors.filter(factor => factor.category === "weakness").map((factor, index) => [index + 1, factor.description, factor.weight.toFixed(2), factor.rating, (factor.weight * factor.rating).toFixed(2)]);
          (doc as any).autoTable({
            head: [["", "Weaknesses", "", "", ""]],
            body: weaknessRows,
            startY: yPosition,
        ...
        // For EFE, separate opportunities and threats
          ...
          const opportunityRows = factors.filter(factor => factor.category === "opportunity").map((factor, index) => [index + 1, factor.description, factor.weight.toFixed(2), factor.rating, (factor.weight * factor.rating).toFixed(2)]);
          (doc as any).autoTable({
            head: [["", "Opportunities", "", "", ""]],
          ...
          // Threats section
          const threatRows = factors.filter(factor => factor.category === "threat").map((factor, index) => [index + 1, factor.description, factor.weight.toFixed(2), factor.rating, (factor.weight * factor.rating).toFixed(2)]);
          (doc as any).autoTable({
            head: [["", "Threats", "", "", ""]],
            body: threatRows,
            startY: yPosition,
      ...
      if (type === "ie" && ifeScore > 0 && efeScore > 0) {
        doc.addPage();
        doc.setFontSize(16);
        doc.text("Internal-External (IE) Matrix", 105, 20, {
          align: "center"
        });
        doc.setFontSize(12);
        doc.text(`IFE Score: ${ifeScore.toFixed(2)} | EFE Score: ${efeScore.toFixed(2)}`, 105, 30, {
          align: "center"
        });
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      if (companyProfile) {
        const profileWorksheet = XLSX.utils.aoa_to_sheet([["Company Profile"], [], ["Company Name", companyProfile.name], ["Industry", companyProfile.industry], ["Description", companyProfile.description], ["Vision", companyProfile.vision], ["Mission", companyProfile.mission]]);
      ...
      if (swotItems.length > 0) {
        ...
        for (let i = 0; i < maxLength; i++) {
          swotData.push([strengths[i]?.description || "", weaknesses[i]?.description || "", opportunities[i]?.description || "", threats[i]?.description || ""]);
        }
      ...
      let matrixData: any[][] = [];
      if (type === "ife") {
        matrixData = [["Internal Factor Evaluation (IFE) Matrix"], [`Generated: ${new Date().toLocaleDateString()}`], [], ["Strengths"], ["#", "Factor Description", "Weight", "Rating", "Weighted Score"]];

        // Add strengths
        const strengthFactors = factors.filter(f => f.category === "strength");
        strengthFactors.forEach((factor, index) => {
          matrixData.push([index + 1, factor.description, factor.weight, factor.rating, factor.weight * factor.rating]);
        });
        matrixData.push([]);
        ...
        // Add total
        matrixData.push([]);
        matrixData.push(["", "TOTAL", factors.reduce((sum, factor) => sum + factor.weight, 0), "", score]);
      } else if (type === "efe") {
        matrixData = [["External Factor Evaluation (EFE) Matrix"], [`Generated: ${new Date().toLocaleDateString()}`], [], ["Opportunities"], ["#", "Factor Description", "Weight", "Rating", "Weighted Score"]];
        ...
        matrixData = [["Internal-External (IE) Matrix"], [`Generated: ${new Date().toLocaleDateString()}`], [], ["Matrix Position"], ["IFE Score", "EFE Score", "Cell Position", "Strategic Implication"]];
      ...
      const interpretationData = [["Score Interpretation"], [], ["IFE Score", ifeScore.toFixed(2), getScoreInterpretation(ifeScore)], ["EFE Score", efeScore.toFixed(2), getScoreInterpretation(efeScore)], [], ["Rating Scale for Internal Factors:"], ["1", "Major Weakness"], ["2", "Minor Weakness"], ["3", "Minor Strength"], ["4", "Major Strength"], [], ["Rating Scale for External Factors:"], ["1", "Poor Response"], ["2", "Average Response"], ["3", "Above Average Response"], ["4", "Superior Response"]];
